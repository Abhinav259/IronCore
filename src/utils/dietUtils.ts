...
export const getMealImage = (description: string, type: string): DietImage => {
  ...
  if (d.includes('oatmeal') || d.includes('oats')) {
    return {
      url: 'https://images.unsplash.com/photo-1517673132405-a56a62b18caf?w=800&auto=format&fit=crop',
  ...
  if (d.includes('egg') || d.includes('omelet') || d.includes('scramble')) {
    if (t.includes('vegan')) {
      return {
        url: 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=800&auto=format&fit=crop',
  ...
  if (d.includes('smoothie') || d.includes('shake')) {
    return {
      url: 'https://images.unsplash.com/photo-1502741224143-90386d7f8c82?w=800&auto=format&fit=crop',
      alt: 'A refreshing protein smoothie with fruits'
    };
  }
  if (d.includes('pancake')) {
    return {
      url: 'https://images.unsplash.com/photo-1528207776546-365bb710ee93?w=800&auto=format&fit=crop',
  ...
  if (d.includes('chicken')) {
    return {
      url: 'https://images.unsplash.com/photo-1532550907401-a500c9a57435?w=800&auto=format&fit=crop',
  ...
  if (d.includes('salmon') || d.includes('fish') || d.includes('cod') || d.includes('tilapia') || d.includes('shrimp')) {
    return {
      url: 'https://images.unsplash.com/photo-1467003909585-2f8a72700288?w=800&auto=format&fit=crop',
  ...
  if (d.includes('pasta') || d.includes('spaghetti') || d.includes('noodles')) {
    return {
      url: 'https://images.unsplash.com/photo-1473093226795-af9932fe5856?w=800&auto=format&fit=crop',
  ...
  if (d.includes('nuts') || d.includes('almonds') || d.includes('walnuts') || d.includes('macadamia')) {
    return {
      url: 'https://images.unsplash.com/photo-1596450514735-111a2fe02935?w=800&auto=format&fit=crop',
  ...
  if (d.includes('fruit') || d.includes('apple') || d.includes('banana') || d.includes('pineapple') || d.includes('berries')) {
    return {
      url: 'https://images.unsplash.com/photo-1610832958506-aa56368176cf?w=800&auto=format&fit=crop',
      alt: 'Fresh and colorful seasonal fruits'
    };
  }
  if (d.includes('hummus') || d.includes('carrot') || d.includes('cucumber')) {
    return {
      url: 'https://images.unsplash.com/photo-1574158622682-e40e69881006?w=800&auto=format&fit=crop',
      alt: 'Fresh vegetable sticks with creamy hummus'
    };
  }
  if (d.includes('cheese') || d.includes('cottage cheese')) {
    return {
      url: 'https://images.unsplash.com/photo-1559561853-08451507cbe7?w=800&auto=format&fit=crop',
  ...
  if (t.includes('keto')) {
    return {
      url: 'https://images.unsplash.com/photo-1603048297172-c92544798d5e?w=800&auto=format&fit=crop',
